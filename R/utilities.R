...
compute.cosine = function(m1, m2, assigned_missing, what, subset_cols=NULL) {
  ...
  m1 = as.data.frame(m1); m2 = as.data.frame(m2)

  if (what == "expos") {
    if (any(rownames(m1) != rownames(m2))) rownames(m2) = rownames(m1)
  ...
  rownames(m1) = paste0("F_", rownames(m1)); rownames(m2) = paste0("S_", rownames(m2))

  compare = rbind(m1[paste0("F_",consider), ], m2[paste0("S_",names(consider)), ] )
