...
counts_all = readRDS(paste0(data_path, "counts_dbs.Rds"))
...
x.fit = fit(x=counts_i,
            ...
            reference_catalogue=COSMIC_dbs[subset_reference,],
	    keep_sigs=c(),
...
x.fit_new = fit(x=counts_i,
                ...
                enforce_sparsity=TRUE,
                dirichlet_prior=TRUE,
                reference_catalogue=COSMIC_dbs[new_sigs,],
		keep_sigs=c(),
                hyperparameters=list("alpha_conc"=100, "scale_factor_alpha"=10000,
                                     "scale_factor_centroid"=10000, "scale_tau"=0),  # change default values to hyperparameters
...
saveRDS(x.fit, paste0(data_path, "all_organs/fit.real_data.dbs.", organ_i, ".N",
                      nrow(counts_i),
                      ".", run_id, ".Rds"))
