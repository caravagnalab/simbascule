...
stats_plots = lapply(c("NoPenalty","PenaltyN"), function(penalty_id) {
  ...
  cosine_expos = make_boxplot(stats_p, "cosine_expos") + labs(title="cosine_expos")
  cosine_sigs = make_boxplot(stats_p, "cosine_sigs") + labs(title="cosine_sigs")
  mse_counts = make_boxplot(stats_p, "mse_counts") + labs(title="mse_counts")

  k_ratio = stats_p %>% dplyr::select(N, G, seed, dplyr::contains("K_")) %>%
